...
document.addEventListener("DOMContentLoaded", function () {
  ...
  chrome.storage.sync.get("extensionEnabled", (data) => {
    ...
    updateBadge(isEnabled); // Update the badge color based on the state
    if (isEnabled) {
      runExtension();
    }
  });
  ...
  extensionToggle.addEventListener("change", function () {
    ...
    chrome.storage.sync.set({ extensionEnabled: isEnabled }, () => {
      ...
      updateBadge(isEnabled); // Update badge when the state changes
    ...
    function fetchItems() {
      ...
      chrome.runtime.sendMessage({ type: "fetch" }, function (response) {
        ...
        if (response && response.items && Array.isArray(response.items)) {
          ...
          sortedItems.forEach((item) => {
            ...
            li.innerHTML = `
              ...
              <div class="item-actions">
                ...
                <button class="quick-chatgpt-btn" data-text="${encodeURIComponent(
                  item.text
                )}">Quick ChatGPT</button>
          ...
          // Add event listeners for new Quick ChatGPT buttons
          document.querySelectorAll(".quick-chatgpt-btn").forEach((btn) => {
            btn.addEventListener("click", function () {
              const text = decodeURIComponent(this.getAttribute("data-text"));
              chrome.runtime.sendMessage({
                type: "useOpenChatGPT",
